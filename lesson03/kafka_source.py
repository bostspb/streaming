...
# Run Pyspark as:
# pyspark --master local --packages org.apache.spark:spark-sql-kafka-0-10_2.11:2.4.7
...
topic = "lesson3_student782_3_topic"
...
raw_orders = spark\
    ...
    .option("subscribe", topic)\
    .option("startingOffsets", "earliest")\
    .option("endingOffsets", """{"orders_topic_json":{"0": 20, "1": 20, "2": 20}}""")\
    .load()
...
out = console_output(raw_orders_stream, 5)
out.stop()
...
raw_orders = spark.readStream. \
    ...
    option("kafka.bootstrap.servers", kafka_brokers). \
    option("subscribe", topic). \
    option("startingOffsets", """{"orders_topic_json":{"0": 20, "1": 20, "2": 20}}"""). \
    option("maxOffsetsPerTrigger", "5"). \
    load()
